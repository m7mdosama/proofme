  ...
  Annotate.prototype = {
    init: function() {
      ...
      if (self.options.bootstrap) {
        ...
        self.$tool += '<label class="btn active myannolabel">' +
          '<input type="radio" name="' + self.toolOptionId +
          '" data-tool="rectangle"' +
          ' data-toggle="tooltip" data-placement="top"' +
          ' title="Draw an rectangle">' +
          '<i class="glyphicon glyphicon-unchecked myrect myanno"></i>' +
          '</label><label class="btn myannolabel">' +
          '<input type="radio" name="' + self.toolOptionId +
          '" data-tool="circle"' +
          ' data-toggle="tooltip"' +
          'data-placement="top" title="Write some text">' +
          '<i class="glyphicon glyphicon-copyright-mark mycyrc myanno"></i>' +
          '</label><label class="btn myannolabel">' +
          '<input type="radio" name="' + self.toolOptionId +
          '" data-tool="text"' +
          ' data-toggle="tooltip"' +
          'data-placement="top" title="Write some text">' +
          '<i class="glyphicon glyphicon-font mytext myanno"></i></label>' +
          '<label class="btn  myannolabel">' +
          '<input type="radio" name="' + self.toolOptionId +
          '" data-tool="arrow"' +
          ' data-toggle="tooltip" data-placement="top" title="Draw an arrow">' +
          '<i class="glyphicon glyphicon-arrow-up myarrow myanno"></i></label>' +
          '<label class="btn  myannolabel">' +
          '<input type="radio" name="' + self.toolOptionId +
          '" data-tool="pen"' +
          ' data-toggle="tooltip" data-placement="top" title="Pen Tool">' +
          '<i class="glyphicon glyphicon-pencil myfreehand myanno"></i></label>' +
          '</div>' +
    ...
    setBackgroundImage: function(image) {
      ...
      if(image.path.endsWith('pdf')){
          self.img = document.createElement("object");
          self.img.data = image.path+"#toolbar=0&navpanes=0&scrollbar=0&view=Fit&pagemode=thumbs";
          self.img.setAttribute('width', self.options.width);
          self.img.setAttribute('height', self.options.height);
          self.$el.append(self.img);
          self.selectImageSize.width =self.currentWidth = self.options.width;
          self.selectImageSize.height =self.currentHeight = self.options.height;
          ...
          self.img = document.createElement('img');//new Image();
          self.img.src = image.path;
      }
      self.img.onload = function() {
          //hamada;
        if ( false && ((self.options.width && self.options.height) !== undefined || (self.options.width && self.options.height) !== 0)) {
